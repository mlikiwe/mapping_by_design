...
def process_optimization(
    ...
    for i, dest_row in df_dest.iterrows():
        ...
        for j, orig_row in df_origin.iterrows():
            ...
            # Hitung cost trucking
            size_cont = dest_row['SIZE CONT']
            size_int = 20 if '20' in str(size_cont) or '21' in str(size_cont) else 40
            
            # Cost via port = dest->port + port->origin
            cost_via_port = (
                calculate_trucking_cost(dest_cabang, size_int, dist_dest_to_port) +
                calculate_trucking_cost(dest_cabang, size_int, dist_port_to_orig)
            )
            
            # Cost triangulasi = dest->origin langsung
            cost_triangulasi = calculate_trucking_cost(dest_cabang, size_int, dist_direct)
