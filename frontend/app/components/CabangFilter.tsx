...
export default function CabangFilter({
  ...
  return (
    <div className="mb-4 flex flex-wrap gap-2">
      {cabangStats.map(({ cabang, match, saving }) => (
        <button
          key={cabang}
          onClick={() => onSelectCabang(selectedCabang === cabang ? 'all' : cabang)}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border ${selectedCabang === cabang
        ...
        >
          ...
          <span className="ml-2 opacity-80">{match} rute</span>
